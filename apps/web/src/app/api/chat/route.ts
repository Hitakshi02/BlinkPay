// Runtime ENV fallback: prefer an external ./env module if available in your environment,
// but default to reading from process.env so the module import is not required.
const ENV = {
  BLOCKSCOUT_MCP_URL: process.env.BLOCKSCOUT_MCP_URL ?? "",
  BLOCKSCOUT_MCP_API_KEY: process.env.BLOCKSCOUT_MCP_API_KEY ?? undefined,
  OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? undefined,
  OPENAI_ORG: process.env.OPENAI_ORG ?? undefined,
  OPENAI_MODEL: process.env.OPENAI_MODEL ?? undefined,
};

/**
 * Fallback prompt definitions for when ./prompts/blockscout is not available.
 * Replace these with the real imports from ./prompts/blockscout if you add that file.
 */
export const SUPER_PROMPT_SYSTEM = `You are a helpful assistant specialized in blockchain explorer data. Provide concise, factual answers and include contextual anchors when available.`;
export const SUPER_PROMPT_TEMPLATE = `Chain: {chain}
Transaction: {txHash}
Address: {address}
Session: {sessionId}
Question: {question}`;
...
export const FEW_SHOTS = [
  {
    user: "What happened with transaction 0xabc123?",
    assistant:
      "Transaction 0xabc123 was included in block 0x1a2b3c, succeeded, and transferred 0.5 ETH.",
  },
  {
    user: "Does address 0xdef456 have any contracts?",
    assistant:
      "Address 0xdef456 is an externally owned account (EOA) with a native balance and several token positions.",
  },
];
